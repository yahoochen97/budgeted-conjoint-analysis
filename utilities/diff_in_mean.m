...
%             dim_std = [dim_std, sqrt(var(tmp1)/numel(tmp1)+var(tmp2)/numel(tmp2))];
       ...
       vs = sort(tmp);
       for k=2:numel(vs)
           tmp1 = (1+train_y(raw_x(:,j)==vs(1)))./2;
           tmp2 = (1+train_y(raw_x(:,j)==vs(k)))./2;
           dim_mu = [dim_mu, (mean(tmp2)-mean(tmp1))]; % ./(vs(k)-vs(1))
...
function [bootstrap_std] = block_bootstrap(n_bootstrap, data1, data2)
    ...
    elseif size(data1,1)==0
        sample1 = zeros(n_bootstrap,size(data2,2));
    else
        sample1 = bootstrp(n_bootstrap,@mean,data1);
    end
    if size(data2,1)==1
        sample2 = data2.*ones(n_bootstrap,1);
    elseif size(data2,1)==0
        sample2 = zeros(n_bootstrap,size(data1,2));
    else
